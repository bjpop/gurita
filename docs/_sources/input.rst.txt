...
Gurita can read data from a named input file or standard input (stdin).
...
.. code-block:: text 
    ...
    gurita in titanic.csv ... 

We use ``...`` to indicate that the remainder of the example Gurita command is omitted for the sake of simplifying the discussion.
...
.. code-block:: text 
    ...
    gurita in titanic.csv + count -x class 
...
The ``in`` command lets you specify the field separator explicitly. This will override any default behaviour that Gurita would otherwise have when choosing what separator to use. 
...
.. code-block:: text 
    ...
    gurita in --sep '\t' example_filename + count -x class 
...
.. note::
    ...
    ``--sep <str>`` forces Gurita to use the specified field separator regardless of the filename extension or contents of the file. 

Input from standard input (stdin) 
---------------------------------

If you don't specify a file name when using the ``in`` command Gurita will assume that the input should be read from standard input (stdin):
...
.. code-block:: text
    ...
    gurita in + count -x class < titanic.csv

In the above example the notation ``< titanic.csv`` causes the contents of the file ``titantic.csv`` to be fed into the standard input of Gurita.
This is called *input redirection*. 

Instead of using input redirection, it is also possible to *pipe* the output from another command to the standard input of Gurita:

.. code-block:: text 

    cat titanic.csv | gurita in + count -x class
...
into the standard input of Gurita.

Reading from standard input is particularly useful when you want to use Gurita as part of a command pipeline: 

.. code-block:: text

    example_command | gurita in + count -x class

Here ``example_command`` is supposed to represent an arbitrary command, possibly itself a series of commands piped together, whose ouput is sent as input into Gurita.

For performance reasons Gurita does not try to detect the format of the input file when reading from standard input. 

Therefore, when reading from standard input, unless otherwise specified, Gurita assumes that the file is in CSV format.

As before, this can be overridden by ``in --sep <str>``, as illustrated in the example below: 

.. code-block:: text

    cat titanic.tsv | gurita in --sep '\t' + count -x class


.. note::

   **Standard input can only be read once in a Gurita command**

   A Gurita command can only read from standard input at most once in a command. An attempt to read from standard input more than once will result in an error: 

   .. code-block:: text

       gurita in + count -x class + in < titanic.csv
       gurita ERROR: stdin may only be used at most once, and only as the first command; exiting
...
.. note::
   ...
   **Standard input can only be read at the start of a command**
   ...
   Gurita will only permit standard input to be read at the start of a command chain. Therefore it is an error to request to read
   from standard input in any position other then the first command in the chain:

   .. code-block:: text

      gurita count -x class + in < titanic.csv
      gurita ERROR: stdin may only be used at most once, and only as the first command; exiting
...
For convenience, if you don't use the ``in`` command explicitly Gurita will assume you wanted to read from standard input in CSV format.

Therefore:

.. code-block:: text

    gurita in + <rest of command>

can be simplified to:

.. code-block:: text

    gurita <rest of command>

In other words, if a Gurita command starts with ``in +`` you can simply omit that part, and Gurita will implicitly read from standard input using CSV format. This saves you a bit of typing and makes the command line tidier.
...
.. code-block:: text
    ...
    cat titanic.csv | gurita count -x class
...
Note carefully that when implicitly reading from standard input Gurita will always assume the input file is in CSV format. If you want to read a different format from standard input you must explicitly specify
...
Gurita optionally supports comments in input data files where the comment starts with a special character, such as a hash, and continues until the end of the line.
...
.. code-block:: text
    ...
    cat iris.csv | gurita in --comment '#' ... 
...
.. code-block:: text
    ...
    gurita in titanic.csv + ... + in iris.csv + ... 
...
.. code-block:: text
   ...
   gurita in titanic.csv + hist -x fare + in iris.csv + count -x species
