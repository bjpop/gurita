...
The next example illustrates Hatch's ability to :ref:`chain commands <command_chain>` together: 
...
In the command line syntax supported by Hatch, the plus sign ``+`` separates each command in a chain. This notation
is fairly novel to Hatch and not something commonly seen in other command line programs.
...
Command chaining allows complex transformations to be combined together and makes Hatch a very powerful tool.
